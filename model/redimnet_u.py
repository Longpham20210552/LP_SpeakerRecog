...
class MultiHeadAttention(nn.Module):
    ...
    # Reshape from (batch_size, seq_len, embed_dim) -> (batch_size, num_heads, seq_len, head_dim)
    ...
    def forward(self,hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        attn_output = torch.bmm(attn_probs, value_states) # -> Multiply attn with original value_states

        attn_output = attn_output.view(bsz, self.num_heads, tgt_len, self.head_dim) # -> shape (batch_size, num_heads, tgt_len, head_dim)
        attn_output = attn_output.transpose(1, 2)                                   # -> shape (batch_size, tgt_len, num_heads, head_dim)

        # Use the `embed_dim` from the config (stored in the class) rather than `hidden_state` because `attn_output` can be
        # partitioned aross GPUs when using tensor-parallelism.
        attn_output = attn_output.reshape(bsz, tgt_len, self.embed_dim)
        # -> reshape (batch_size, tgt_len, embed_dim) 
        # attn_output has the same shape with the input of MHA layer
        attn_output = self.out_proj(attn_output) # Why we need this layer???
        return attn_output
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        hidden_states = attn_residual + hidden_states   # Add: Residual Block
        hidden_states = self.layer_norm(hidden_states)  # Norm: LayerNorm
...
class MultiScaleAttentionModule(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # Downsampling with 3x1 convolution, stride 2
        self.downsample = nn.Conv1d(in_channels, out_channels, kernel_size=3, stride=2, padding=1)

        # 1x1 Conv for channel reduction before larger kernels
        ...
        self.se = nn.Sequential(
            ...
            nn.Conv1d(out_channels * 4, out_channels * 4 // 16, kernel_size=1),
            nn.ReLU(),
            nn.Conv1d(out_channels * 4 // 16, out_channels * 4, kernel_size=1),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        out1 = self.bn_relu(self.conv1(x))
        out3 = self.bn_relu(self.conv3(x))
        out5 = self.bn_relu(self.conv5(x))
        out7 = self.bn_relu(self.conv7(x))
...
class MultiScaleAttentionModule2D(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(MultiScaleAttentionModule2D, self).__init__()

        # Downsampling với 3x1 Conv, stride 2
        self.downsample = nn.Conv2d(in_channels, out_channels, kernel_size=(3, 1), stride=(2, 1), padding=(1, 0))

        # 1x1 Conv để giảm chiều kênh
    ...
    def forward(self, x):
        x = self.downsample(x)
        # Apply các nhánh Conv song song
        out1 = self.bn_relu(self.conv1(x))
        out3 = self.bn_relu(self.conv3(x))
        out5 = self.bn_relu(self.conv5(x))
        out7 = self.bn_relu(self.conv7(x))

        # Nối các đầu ra theo chiều channel
        out = torch.cat([out1, out3, out5, out7], dim=1)
        se_weight = self.se(out)
        out = out * se_weight
        return out
...
class ResBasicBlock(nn.Module):
    def __init__(self, inc, outc, num_freq, stride=1, se_channels=64, Gdiv=1, use_fwSE=False):
        super().__init__()
        # if inc//Gdiv==0:
        #     Gdiv = inc
...
class ConvBlock2d(nn.Module):
    def __init__(self, c, f, block_type="convnext_like", Gdiv=1):
        ...
        elif block_type == "MSA_2D":
            self.conv_block = MultiScaleAttentionModule2D(c, c)
        else:
...
class TimeContextBlock1d(nn.Module): #Khối 1d
    def __init__(self, 
        C, #CxF
        hC, #CxF//att
        pos_ker_sz = 59,
        ...
        if block_type == 'fc':
            ...
            self.tcm = MultiScaleAttentionModule(hC, hC)
        else:
            raise NotImplemented()
...
class ReDimNet(nn.Module):
    def __init__(self,
        ...
        stages_setup = [
            ...
            (1,2,1,[(3,3)],None), # 16 # 1,2,1,[3,3],12
            (2,3,1,[(3,3)],None), # 32 # 
    ...
    def build(self,F,C,stages_setup,group_divisor,out_channels):
        ...
        self.inputs_weights = torch.nn.ParameterList([
            nn.Parameter(torch.ones(1,1,1,1),requires_grad=True)]+[
